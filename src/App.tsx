...
type View = 'study' | 'practice' | 'exam' | 'review';
interface Session { id: string; mode: Mode; taskId: number; questions: Question[]; responses: Response[] }
const labels: Record<View, string> = { study: 'Estudiar', practice: 'Practicar', exam: 'Mi progreso', review: 'Mis fallos' };
...
function SourceAnswer({ q }: { q: Question }) {
  return <details className="answer"><summary><span className="code">{q.id}</span> {q.prompt}</summary><p><strong>Respuesta del archivo:</strong> {q.answer}</p><small>{tasks[q.taskId - 1].source} · línea {q.line}</small></details>;
}
export default function App() {
  ...
  function start(mode: Mode, pool = task.questions) {
    ...
    const questions = sample(pool, mode === 'exam' ? EXAM_SIZE : size);
    if (!questions.length) return;
    setSession({ id: crypto.randomUUID(), mode, taskId, questions, responses: [] });
    setInput(''); setSubmitted(null); setResult(null); setLeaving(false);
  }
