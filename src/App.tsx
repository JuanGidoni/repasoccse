...
import { availableQuestions, correctAnswer, decodeProgress, EXAM_SIZE, finishAttempt, freshProgress, PASS_RATE, recordResponse, sample, STORAGE_KEY, unlocked, type Attempt, type Mode, type Progress, type Response } from './engine';
...
export default function App() {
  ...
  const readyQuestions = availableQuestions(task.questions);
  const pendingChoices = task.questions.filter(q => q.choiceIssue);
  ...
  function start(mode: Mode, pool = task.questions) {
    ...
    const eligible = availableQuestions(pool);
    if (mode === 'exam' && eligible.length < EXAM_SIZE) return;
    const questions = sample(eligible, mode === 'exam' ? EXAM_SIZE : size).map(q => ({ ...q, options: q.options.length === 2 ? [...q.options] : sample(q.options, q.options.length) }));
  ...
  return <div className="app-shell">
    ...
    <main id="main" className="main">
      ...
      {pendingChoices.length > 0 && <p className="notice" role="status">Las preguntas {pendingChoices.map(q => q.id).join(', ')} tienen alternativas pendientes de revisión y se excluyen de los intentos. La lectura y el resto de preguntas siguen disponibles.</p>}
